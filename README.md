...
- **Data Persistence**: MariaDB data is stored persistently in the `data/` folder.
...
- **`mariadb.cnf`**: MariaDB configuration (e.g., database settings).
...
docker-compose logs
```
- Ensure no other services are running on the ports used by the containers (e.g., 90 for Nginx and 8080 for Adminer).
